...
TcpStatsdSink::TcpStatsdSink(const LocalInfo::LocalInfo& local_info,
                             ...
                             Stats::Scope& scope)
    : local_info_(local_info), cluster_name_(cluster_name), tls_(tls.allocateSlot()),
      cx_overflow_stat_(scope.counter("statsd.cx_overflow")) {}

void TcpStatsdSink::initialize(Upstream::ClusterManager& cluster_manager) {
  cluster_manager_ = &cluster_manager;
  Config::Utility::checkClusterAndLocalInfo("tcp statsd", cluster_name_, cluster_manager,
                                            local_info_);
  cluster_info_ = cluster_manager.get(cluster_name_)->info();
